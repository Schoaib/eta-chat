...
function socialPerksLogin(sender, cb) {
  ...
  client.post(API_URL + '/Customer/FBLogin', args, function(data, response) {
    if (data.token) {
      console.log('data', data)
      spTokens.set(sender, data.token);
      spIds.set(sender, data._id);
      return cb();
    } else {
      args = {
        data: {
      ...
      client.post(API_URL + '/Customer/Login', args, function(data, response) {
        if (data.token) {
          spTokens.set(sender, data.token);
          spIds.set(sender, data._id);
          console.log('data', data)
...
function checkSocialPerksLogin(sender, cb) {
  if (spIds.has(sender) && spTokens.has(sender)) {
    var client = new Client();
    var args = {
      data: {
        "language": "EN",
    ...
    client.post(API_URL + '/Customer/Settings', args, function(data, response) {
      if (!data.errors) {
        return cb();
      } else {
        socialPerksLogin(sender, cb)
      }
    });
  } else {
    socialPerksLogin(sender, cb)
...
function snipCoupon(sender, campaignId, cb) {
  var client = new Client();
  var args = {
    data: {
      "language": "EN",
  ...
  client.post(API_URL + '/Campaign/Snip', args, function(data, response) {
    if (!data.errors) {
      return cb(null);
    } else {
...
function transactionDetails(sender, transactionId, cb) {
    checkSocialPerksLogin(sender, function() {
        ...
        client.post(API_URL + '/Transaction/findOne', args, function(data, response) {
            if (!data.errors) {
                ...
                if (!firstNames.has(sender)) {
                    ...
                    name = firstNames.get(sender);
                }

                var message;
                console.log('data', data)
                console.log('usedVouchers', data.usedVouchers)
                if (data.usedVouchers && data.usedVouchers.length > 0) {

                    campaignDetailsForTransaction(sender, data.usedVouchers[0].sourceCampaignId, function(err, sourceCampaign) {
                        console.log('sourceCampaign', sourceCampaign)
                        message = {
                            attachment: {
                                type: "template",
                                payload: {
                                    ...
                                    elements: [{
                                        title: data.business_unit + ' | Transaction No ' + data.transactionId,
                                        subtitle: 'Bill Amount ' + data.totalPrice.amount + ' ' + data.totalPrice.currency + ' | Paid Amount ' + data.owed.amount + ' ' + data.owed.currency,
                                        image_url: (sourceCampaign.image != 'undefined' && sourceCampaign.image != '') ? sourceCampaign.image : 'https://d13mc760e5f146.cloudfront.net/app/img/logos/' + data.usedVouchers[0].client + 'POS',
                                        buttons: [{
                                            type: "web_url",
                                            title: "Rate Now",
                                            url: LOGIN_API + "rate?campaignId=" + utils.encrypt(data.usedVouchers[0].sourceCampaignId) + '&sender=' + utils.encrypt(sender),
                                        }]
                                    }]
...
//               merchant_name: data.business_unit,
//               currency: data.owed.currency,
//               payment_method: "N/A",
//               timestamp: Math.floor((new Date(data.date)).getTime() / 1000),
//               elements: [
//                 {
//                   title: data.usedVouchers[0].msg,
//                   price: 0,
//                   image_url: 'https://d13mc760e5f146.cloudfront.net/app/img/logos/' + data.usedVouchers[0].client + 'POS'
//                 }
//               ],
//               summary: {
//                 total_cost: data.owed.amount
//               },
//               // adjustments:[
//               //   {
...
function campaignDetails(sender, campaignId, cb) {
  var client = new Client();
  var args = {
    data: {
      "language": "EN",
  ...
  client.post(API_URL + '/Campaign/GetCrossCampaignDetails', args, function(data, response) {
    if (!data.errors) {
      ...
      if (data.snippable) {
        redeemButton = {
          type: "postback",
          title: "Save Perk",
          payload: "SNIP|" + data._id
        };
      } else {
        redeemButton = {
          type: "web_url",
          title: "Redeem Now",
          url: LOGIN_API + "pin?participate=true&clientName=" + utils.encrypt(data.redeemers[0].clientName) + "&outletName=" + utils.encrypt(data.redeemers[0].name) + "&campaignId=" + utils.encrypt(data._id) + "&recipientId=" + utils.encrypt(sender) + "&name=" + utils.encrypt(data.title) + "&userId=" + utils.encrypt(spIds.get(sender)),
                ...
                buttons: [
                  {
                    ...
                    url: LOGIN_API + "terms?campaignId=" + utils.encrypt(data._id) + '&sender=' + utils.encrypt(sender)
                  }
                ]
              }
            ]
...
function searchCampigns(sender, text) {
  checkSocialPerksLogin(sender, function() {
    ...
    client.post(API_URL + '/Campaign/FindElligibleCampaigns', args, function(data, response) {
      data = data.campaigns;
      console.log(data);
      var elements = [];
      async.eachSeries(data, function(result, doneWithRole) {
        ...
        if (elements.length > 0) {
          ...
          sendFBMessage(sender, {text: 'No results found'});
        }
...
function processEvent(event) {
  ...
  if ((event.message && event.message.text) || (event.postback && event.postback.payload)) {
      ...
      if (isDefined(response.result)) {
        ...
        if (isDefined(responseData) && isDefined(responseData.facebook)) {
          ...
          switch (response.result.metadata.intentName) {
            ...
            case 'Clerk_Transaction':
              location.set(sender, 'CLERK_TRANSACTION_HISTORY');
              sendFBMessage(sender, {
          ...
          switch (response.result.metadata.intentName) {
            ...
            case 'Search':
              ...
              console.log('response.result.parameters.category', response.result.parameters.category)

              if (response.result.parameters.category != '') {
                searchCampigns(sender, response.result.parameters.category);
              } else {
                sendFBMessage(sender, {text: 'Please be more specific'});
              }
              break;
            case "NEARBY":
              checkSocialPerksLogin(sender, function() {
    ...
    if (location.get(sender) == 'SHOW_PUBLIC_DEALS_NEARBY') {
        checkSocialPerksLogin(sender, function() {
      if (spIds.get(sender) == '574f2ce553cbb7350313533f') {
        loginSignup(sender);
      } else {
        ...
        client.post(API_URL + '/Campaign/GetMyElligibleCampaigns', args, function(data, response) {
          ...
          async.eachSeries(data, function(result, doneWithRole) {
            ...
            if (result.endDate != 'undefined' && result.endDate !='') {
              subTitle += ' | Save before: ' + new Date(result.endDate).toLocaleDateString('en-GB', {
                day: 'numeric',
                month: 'short'
              }).split(' ').join(' ');
            }
            ...
            var redeemButton;
            var distance = [];
            if (result.redeemers[0].pos) {
              var geoloc = require('geolocation-distances');
              var userLatLong = {
              ...
              if (result.redeemers[0].pos[0])
                distance.push(geoloc.getDistance(userLatLong, result.redeemers[0].pos[0]));
            ...
            if (result.snippable) {
              ...
              redeemButton = {
                type: "web_url",
                title: "Redeem",
                url: LOGIN_API + "pin?clientName=" + utils.encrypt(result.redeemers[0].clientName) + "&outletName=" + utils.encrypt(result.redeemers[0].name) + "&campaignId=" + utils.encrypt(result._id) + "&recipientId=" + utils.encrypt(sender) + "&name=" + utils.encrypt(result.title) +
                "&userId=" + utils.encrypt(spIds.get(sender)),
                webview_height_ratio: "full",
                messenger_extensions: true
              };
            }
            ...
            result.distance = _.min(distance, function(o) {
              return o.val;
            });

            if (parseInt(result.distance) < 1) {
              subTitle += " - " + parseInt(result.distance.toFixed(2) * 100) + " m";
            } else {
              subTitle += " - " + parseInt(result.distance) + " km";
            }
            ...
            var newElements = _.map(elements, function(o) {
            ...
            var messageData = {
              attachment: {
                ...
                payload: {
                  template_type: "generic",
                  elements: newElements.slice(0, 9)
                }
              }
            };
            ...
            if (elements.length > 0) {
                sendFBMessage(sender, {
                    text: 'Check out these great offers near you!'
                });
    ...
    } else if (location.get(sender) == 'CLERK_TRANSACTION_HISTORY') {
      var messageData = {
        attachment: {
          type: "template",
          payload: {
            template_type: "button",
            text: "Please login to view transaction history",
            buttons: [
              {
                type: "web_url",
                ...
                "webview_height_ratio": "full",
                "messenger_extensions": true,
                "url": LOGIN_API + "transaction_login?lat=" + event.message.attachments[0].payload.coordinates.lat + "&lon=" + event.message.attachments[0].payload.coordinates.long
              }
            ]
          }
        }
...
subtitle: 'Recent Transactions'
    ...
    default:
      ...
      //         };
      //
      //         client.post(API_URL + '/Campaign/getCrossCampaignsByMyIssuers', args, function(data, response) {
      //             console.log(data);
      //             var elements = [];
      //             async.eachSeries(data, function(result, doneWithRole) {
      //                 console.log('result.redeemers', result.redeemers)
      //                 elements.push({
      //                     title: result.title,
      //                     subtitle: '🏪 Outlet: ' + result.redeemers[0].name + ' | Expires on: ' + new Date(result.endDate).toLocaleDateString('en-GB', {
      //                         day: 'numeric',
      //                         month: 'short',
      //                         year: 'numeric'
      //                     }).split(' ').join(' ') + ' | Inventory Left : ' + result.inventoryLeft,
